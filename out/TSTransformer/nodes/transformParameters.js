...
function transformParameters(state, node) {
    ...
    for (const parameter of node.parameters) {
        ...
        if (parameter.dotDotDotToken) {
            hasDotDotDot = true;
            varArgsData = (0, varArgsOptimization_1.analyzeVarArgsOptimization)(state, node.body, parameter, paramId);
            (0, varArgsOptimization_1.handleVarArgsParameterOptimization)(statements, varArgsData, paramId);
        }
        else {
            luau_ast_1.default.list.push(parameters, paramId);
        }
