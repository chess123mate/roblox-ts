...
function transformEnumDeclaration(state, node) {
    if (typescript_1.default.hasSyntacticModifier(node, typescript_1.default.ModifierFlags.Const) && state.compilerOptions.preserveConstEnums !== true) {
        return luau_ast_1.default.list.make();
    }
    const symbol = state.typeChecker.getSymbolAtLocation(node.name);
    if (symbol &&
        (0, hasMultipleDefinitions_1.hasMultipleDefinitions)(symbol, declaration => typescript_1.default.isEnumDeclaration(declaration) && !typescript_1.default.hasSyntacticModifier(declaration, typescript_1.default.ModifierFlags.Const))) {
